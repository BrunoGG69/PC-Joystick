...
def keyUp(key):  # what to do if key released. takes value from handleJoyStickAsArrowKeys
    if key in keysDown:
        ...
        # runs pydirectinput using key from (argument)
        pydirectinput.keyUp(key)
        # print('Up: ', key)         #remove '#' from print to test data stream


# note that the x and y directions are swapped due to the way I orient my thumbstick
def handleJoyStickAsArrowKeys(x, y, z):
    if x == 2:  # 0 is up on joystick
        keyDown('up')  # add up key to keyDown (argument)
        # add down key to keyUp (argument), as you can't press up and down together
        keyUp('down')
    ...
    if y == 0:  # 2 is right on joystick
        keyDown('right')
        keyUp('left')
    elif y == 2:  # 0 is left on joystick
        keyDown('left')
        keyUp('right')
    else:  # 1 is neutral on joystick
        keyUp('left')
...
while True:
    ...
    if data.startswith("S"):  # make sure the read starts in the correct place
        # X direction is second digit in data (data[0] is 'S')
        dx = int(data[1])
        ...
        # run body of code using dx, dy and JSButton as inputs
